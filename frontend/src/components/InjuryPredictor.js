...
function InjuryPredictor() {
  ...
  // Frontend display labels (user-friendly)
  const displayLabels = {
    Age: 'Player Age',
    ...
    DateOfInjury: 'Date of Previous Injury',
    DateOfReturn: 'Date of Previous Injury Return'
  };

  // Backend key mapping (actual keys expected by the backend)
  const backendKeyMapping = {
    Age: 'Age',
    TotalMinutesPlayed: 'total_minutes_played',
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = Object.keys(formData).reduce((acc, key) => {
      const backendKey = backendKeyMapping[key];
      acc[backendKey] = formData[key];
      return acc;
    }, {});
  ...
  return (
    <div style={{ marginTop: '70px' }}> {/* Added marginTop here to avoid touching the header */}
      <div style={{ backgroundColor: 'white', color: 'black', fontFamily: 'Arial, sans-serif', padding: '40px', borderRadius: '8px', boxShadow: '0 4px 10px rgba(0, 0, 0, 0.1)', maxWidth: '700px', margin: 'auto', marginBottom: '100px' }}>
        <h2 style={{ textAlign: 'center', fontSize: '28px', marginBottom: '30px', color: '#333' }}>Injury Type Predictor</h2>
        <form onSubmit={handleSubmit}>
          {Object.entries(formData).map(([key, value]) => (
            <div key={key} style={{ marginBottom: '20px' }}>
              <label htmlFor={key} style={{ display: 'block', fontSize: '16px', marginBottom: '5px', color: '#333' }}>
                {displayLabels[key]} {/* Display user-friendly labels */}
              </label>
              <input
                type="text"
                name={key}
